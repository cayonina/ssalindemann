...
class ProfView extends StatefulWidget {
  ...
  const ProfView({Key? key, required this.uid}) : super(key: key);
...
class _ProfViewState extends State<ProfView> {
  ...
  void initState() {
    super.initState();
    final profesoresProvider =
        Provider.of<ProfesorProvider>(context, listen: false);
    final userFormProvider =
        Provider.of<UserFormProvider>(context, listen: false);

    profesoresProvider.getProfesorById(widget.uid).then((userDB) {
      if (userDB != null) {
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (user != null) _profesorViewBody(),
          Text('${user?.nombres}')
        ],
      ),
    );
...
class _profesorViewBody extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
      child: Table(
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                decoration: CustomInputs.formInputDecoration(
                    hint: 'Area', label: 'Area', icon: Icons.map_outlined),
                validator: (value) {
                  if (value == null || value.isEmpty)
                    return 'Ingrese el area en el que se especializo';
                  return null;
                },
              ),
              SizedBox(
              ...
              ConstrainedBox(
                ...
                child: ElevatedButton(
                    onPressed: () async {
                      ...
                      final saved = await userFormProvider.updateProfesor();
                      if (saved) {
